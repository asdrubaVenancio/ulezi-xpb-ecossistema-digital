...
const listPublicJobs = async (req, res) => {
  try {
    ...
    let where = 'WHERE j.status = "aprovada" AND (j.expires_at IS NULL OR j.expires_at > NOW())';
    const params = [];
    ...
    if (search) {
      where += ' AND (j.titulo LIKE ? OR j.descricao LIKE ? OR j.localizacao LIKE ?)';
...
const createJob = async (req, res) => {
  try {
    ...
    if (!companyProfile) {
      return error(res, 'Empresa nao esta aprovada ou a assinatura expirou.', 403);
    }
    ...
    return success(res, { id: result.insertId }, 'Vaga publicada com sucesso.', 201);
  } catch (err) {
...
/**
 * GET /api/admin/jobs
 * Lista todas as vagas para gestão administrativa.
 */
const adminListJobs = async (req, res) => {
  try {
    ...
    const [vagas] = await pool.execute(
      `SELECT j.*, cp.nome_empresa, cp.sector, ua.nome as aprovado_por_nome
       FROM company_job_postings j
       LEFT JOIN company_profiles cp ON cp.id = j.company_id
