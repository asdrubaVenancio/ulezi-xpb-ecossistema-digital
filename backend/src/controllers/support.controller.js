...
const listAllTickets = async (req, res) => {
  try {
    ...
    if (termoPesquisa) {
      whereClause +=
        " AND (st.assunto LIKE ? OR st.ticket_number LIKE ? OR u.nome LIKE ? OR u.email LIKE ?)";
      params.push(
        `%${termoPesquisa}%`,
        `%${termoPesquisa}%`,
        `%${termoPesquisa}%`,
...
const createTicket = async (req, res) => {
  try {
    ...
    if (!assunto || !categoria || !descricao) {
      return badRequest(
        res,
        "Assunto, categoria e descrição são obrigatórios.",
      );
    }
...
const addMessage = async (req, res) => {
  try {
    ...
    if (tk.status === "fechado" && !isAdmin) {
      return badRequest(
        res,
        "Não é possível adicionar mensagens a um ticket fechado.",
      );
    }
