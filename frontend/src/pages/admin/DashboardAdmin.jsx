...
const SECAO_META = {
  painel: { label: 'Painel geral', descricao: 'Indicadores e atalhos para o que precisa de atenção hoje.' },
  notificacoes: { label: 'Notificações', descricao: 'Alertas operacionais e mensagens por tratar.' },
  utilizadores: { label: 'Utilizadores', descricao: 'Contas registadas, papéis e estados de acesso.' },
  cursos: { label: 'Cursos', descricao: 'Catálogo, categorias e disponibilidade para inscrições.' },
  centros: { label: 'Centros', descricao: 'Centros de formação ligados ao programa.' },
  ofertas: { label: 'Ofertas', descricao: 'Ofertas por curso e ciclos de inscrição.' },
  inscricoes: { label: 'Inscrições', descricao: 'Pedidos dos alunos, aprovação e acompanhamento.' },
  empresas: { label: 'Empresas', descricao: 'Registo, documentação e aprovação de perfis empresariais.' },
  oportunidades: { label: 'Oportunidades', descricao: 'Oportunidades de negócio publicadas pelas empresas.' },
  interesses: { label: 'Interesses', descricao: 'Manifestações de interesse dos investidores.' },
  investimentos: { label: 'Investimentos', descricao: 'Processos de investimento e respetivos estados.' },
  mediacao: { label: 'Mediação', descricao: 'Processos entre empresas e investidores.' },
  contratos: { label: 'Contratos', descricao: 'Contratos gerados e histórico documental.' },
  assinaturas: { label: 'Assinaturas', descricao: 'Planos empresariais e estado de subscrição.' },
  funcionarios: { label: 'Funcionários', descricao: 'Equipa interna e perfis operacionais.' },
  consultoria: { label: 'Consultoria', descricao: 'Pedidos de consultoria e acompanhamento.' },
  ...
  pagamentos: { label: 'Pagamentos', descricao: 'Comprovativos e conciliação financeira.' },
  vagas: { label: 'Vagas (empresas)', descricao: 'Vagas publicadas e estado de aprovação.' },
  ficheiros: { label: 'Ficheiros', descricao: 'Repositório de anexos e documentos.' },
  seguranca: { label: 'Segurança', descricao: 'Controlo de acesso e boas práticas.' },
  configuracoes: { label: 'Configurações', descricao: 'Parâmetros gerais da plataforma.' },
};
...
export default function DashboardAdmin({ secaoInicial = 'painel' }) {
  ...
  const hubActivo = HUBS_ADMIN.find((hub) => hub.secoes.includes(secaoActiva)) || HUBS_ADMIN[0];
    ...
    <div className="admin-layout">
      ...
      <main className="admin-main">
        <header className="admin-topbar">
          <button type="button" id="sidebar-toggle" className="admin-topbar__icon-btn"
        ...
        <div className="admin-page">
          <div className="admin-shell">
            ...
            <div className="admin-hub-tabs" role="tablist" aria-label={`Secções em ${hubActivo.label}`}>
              {hubActivo.secoes.map((secaoId) => {
                const item = SECAO_META[secaoId] || { label: secaoId, descricao: '' };
                ...
                return (
                  ...
                  >
                    ...
                    <span className="admin-hub-tabs__copy">{item.descricao}</span>
                  </button>
                );
              })}
            </div>

            <SecaoActual navegarSecao={irParaSecao} setNotifCount={setNotifCount} />
          </div>
        </div>
      </main>
