  ...
  return (
    <div className="dashboard">
      ...
      <div className="stats-grid">
        <StatCard icone={<Users size={20} color="var(--ciano)"/>} label="Utilizadores" valor={stats?.total_utilizadores?.toLocaleString('pt-AO') || '0'} corIcone="var(--ciano-100)" />
        <StatCard icone={<BookOpen size={20} color="var(--laranja)"/>} label="Inscrições" valor={stats?.total_inscricoes?.toLocaleString('pt-AO') || '0'} corIcone="var(--laranja-100)" />
        <StatCard icone={<Building2 size={20} color="var(--verde)"/>} label="Empresas" valor={stats?.total_empresas?.toLocaleString('pt-AO') || '0'} corIcone="var(--verde-100)" />
        <StatCard icone={<CreditCard size={20} color="var(--vermelho)"/>} label="Pag. pendentes" valor={stats?.pagamentos_pendentes?.toLocaleString('pt-AO') || '0'} corIcone="var(--vermelho-100)" alerta={stats?.pagamentos_pendentes > 0} />
      </div>
      ...
      <div className="dashboard-summary-grid">
        ...
        <div className="card" style={{ padding: 24 }}>
          ...
          <p style={{ fontSize: '0.8rem', color: 'var(--txt-3)', marginBottom: 20 }}>Estado atual do ecossistema</p>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            {[
              ...
              { label: 'Op. em anlise',         valor: stats?.oportunidades_pendentes || 0, cor: 'var(--roxo)' },
            ].map(item => (
                ...
                {inscricoesPend.map(i => (
                  <div key={i.id} style={{ border: '1px solid var(--border)', borderRadius: 'var(--r-md)', padding: 10, background: 'var(--bg-2)' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                      ...
                      <span className="badge badge--amarelo" style={{ fontSize: '0.7rem' }}>Inscrio</span>
                    </div>
                    <p style={{ fontSize: '0.75rem', color: 'var(--txt-3)', marginBottom: 6 }}>
                      {i.nome_curso}  {formatData(i.created_at)}
                    </p>
                    <button type="button" className="btn btn--primary btn--sm btn--full" onClick={() => navegarSecao?.('inscricoes')}>
                      Analisar inscrio
                    </button>
                  </div>
                ))}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {empresasPend.map(e => (
                  <div key={e.id} style={{ border: '1px solid var(--border)', borderRadius: 'var(--r-md)', padding: 10 }}>
                    ...
                    <button type="button" className="btn btn--primary btn--sm btn--full" onClick={() => navegarSecao?.('empresas')}>
                      Analisar empresa
                    </button>
                  </div>
                ))}
  ...
  const alterarStatus = useCallback(async (id, novoStatus) => {
    const ok = await toast.confirmar({
      ...
      mensagem: novoStatus === 'bloqueado'
        ? 'O utilizador não conseguir entrar na plataforma. Continuar?'
        : 'O utilizador voltar a ter acesso à plataforma.',
    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
        ...
        <select className="form-select" style={{ width: 160 }} value={filtroRole} onChange={e => setFiltroRole(e.target.value)}>
          <option value="">Todos os papis</option>
          <option value="estudante">Estudante</option>
          <option value="empresa">Empresa</option>
          <option value="investidor">Investidor</option>
          <option value="funcionario">Funcionrio</option>
        </select>
      </div>
            ...
            <tbody>
              {utilizadores.length === 0 ? (
                ...
                <tr key={u.id}>
                  ...
                  <td>
                    <button type="button" className="btn btn--ghost btn--sm" onClick={() => setModalUser(u)} title="Ações" aria-label={`Ações para ${u.nome}`}>
      ...
      <Modal aberto={!!modalUser} onFechar={() => setModalUser(null)} titulo={`Conta — ${modalUser?.nome}`}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {modalUser?.status !== 'ativo' && (
            <button type="button"  className="btn btn--primary btn--full" onClick={() => alterarStatus(modalUser.id, 'ativo')}>
              <Check size={15}/> Activar utilizador
            </button>
          )}
          {modalUser?.status !== 'bloqueado' && (
            <button type="button"  className="btn btn--danger btn--full" onClick={() => alterarStatus(modalUser.id, 'bloqueado')}>
              <X size={15}/> Bloquear utilizador
            </button>
          )}
          {modalUser?.status !== 'inativo' && (
            <button type="button"  className="btn btn--secondary btn--full" onClick={() => alterarStatus(modalUser.id, 'inativo')}>
              Desactivar conta
            </button>
          )}
        </div>
...
export function Cursos() {
  ...
  const toggleActivo = async (curso) => {
    const ok = await toast.confirmar({
      ...
      mensagem: curso.ativo ? 'Este curso ficar indisponível para inscrições.' : 'Este curso ficar visível para inscrições.',
  ...
  return (
    <div>
      <div className="admin-toolbar admin-toolbar--split">
        <span className="admin-toolbar__hint">Catálogo e disponibilidade para inscrições</span>
      ...
      {carregando ? <PageLoader /> : (
        <div className="table-container">
          <table>
            <thead>
              <tr><th>Nome</th><th>Categoria</th><th>Estado</th><th>Ações</th></tr>
            </thead>
            <tbody>
              {cursos.length === 0 ? (
                ...
                <tr key={c.id}>
                  ...
                  <td>
                    <div style={{ display: 'flex', gap: 6 }}>
                      <button type="button"  className="btn btn--secondary btn--sm" onClick={() => abrirModal(c)}>Editar</button>
                      <button type="button"  className={`btn btn--sm ${c.ativo ? 'btn--danger' : 'btn--primary'}`} onClick={() => toggleActivo(c)}>
      ...
      <Modal aberto={modalAberto} onFechar={() => setModalAberto(false)}
        ...
        acoes={<>
          ...
          <button type="button"  className={`btn btn--primary${enviando?' btn--loading':''}`} onClick={guardar} disabled={enviando}>
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div className="form-group">
            ...
            <input className="form-input" value={form.nome} onChange={e => setForm(f => ({ ...f, nome: e.target.value }))} placeholder="Ex: Informtica bsica" />
          </div>
          <div className="form-group">
  ...
  return (
    <div>
      ...
      <div className="tabs" style={{ marginBottom: 16 }}>
        <button type="button"  className={`tab-btn${abaActiva==='todas'?' active':''}`} onClick={() => setAbaActiva('todas')}>Todas</button>
        <button type="button"  className={`tab-btn${abaActiva==='pendentes'?' active':''}`} onClick={() => setAbaActiva('pendentes')}>
            ...
            <tbody>
              {empresas.length === 0 ? (
                ...
                <tr key={e.id}>
                  ...
                  <td>
                    <button type="button" className="btn btn--ghost btn--sm" onClick={() => { setModalEmp(e); setMotivo(''); }} aria-label={`Opções — ${e.nome_empresa}`}><MoreHorizontal size={18} /></button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <Modal aberto={!!modalEmp} onFechar={() => setModalEmp(null)} titulo={`Empresa — ${modalEmp?.nome_empresa}`}>
        ...
        {modalEmp?.status_aprovacao === 'pendente' ? (
          <>
            <button type="button"  className="btn btn--primary btn--full" style={{ marginBottom: 8 }} onClick={() => aprovar(modalEmp.id, true)}>
            ...
            <button type="button"  className="btn btn--danger btn--full" onClick={() => aprovar(modalEmp.id, false)}>
  ...
  return (
    <div>
      <div className="admin-toolbar admin-toolbar--split admin-toolbar--wrap">
        <p className="admin-toolbar__hint" style={{ maxWidth: 'min(100%, 420px)', margin: 0 }}>
          Validar documentao e decidir aprovação com contexto completo.
        </p>
        <div className="admin-toolbar__actions">
          <div className="form-input-wrapper" style={{ minWidth: 200, flex: '1 1 200px' }}>
            <Search size={16}/>
            <input
      ...
      <div className="tabs" style={{ marginBottom: 16 }}>
        ...
        <button type="button"  className={`tab-btn${abaActiva === 'aprovadas' ? ' active' : ''}`} onClick={() => setAbaActiva('aprovadas')}>Aprovadas</button>
        <button type="button"  className={`tab-btn${abaActiva === 'rejeitadas' ? ' active' : ''}`} onClick={() => setAbaActiva('rejeitadas')}>Rejeitadas</button>
      </div>
            ...
            <tbody>
              {empresas.length === 0 ? (
                ...
                <tr key={e.id}>
                  ...
                  <td style={{ color: 'var(--txt-3)', fontSize: '0.8rem' }}>{formatData(e.criado_em)}</td>
                  <td>
                    <button type="button"  className="btn btn--ghost btn--sm" onClick={() => abrirAnalise(e)}>
              ...
              <div className="company-review-panel">
                ...
                <div className="company-review-actions">
                  ...
                  {estadoEmpresa === 'pendente' ? (
                    <>
                      <button type="button"  className={`btn btn--primary btn--full${enviandoAcao ? ' btn--loading' : ''}`} onClick={() => decidirEmpresa(empresaSelecionada.id, true)} disabled={enviandoAcao}>
                        {!enviandoAcao && <><Check size={15}/> Aprovar empresa</>}
                      </button>
                      <button type="button"  className={`btn btn--danger btn--full${enviandoAcao ? ' btn--loading' : ''}`} onClick={() => decidirEmpresa(empresaSelecionada.id, false)} disabled={enviandoAcao}>
  ...
  return (
    <div>
      <div className="admin-toolbar admin-toolbar--end" style={{ marginBottom: 16 }}>
        <button type="button" className="btn btn--secondary btn--sm" onClick={() => carregar(meta.pagina || 1)}>
          <RefreshCw size={14}/> Actualizar
        </button>
      </div>
      ...
      <div className="stats-grid" style={{ marginBottom: 20 }}>
        ...
        <div className="card" style={{ padding: '20px 24px' }}>
          ...
          <div style={{ fontSize: '0.8rem', color: 'var(--txt-3)' }}>Aguardam Validao</div>
        </div>
        <div className="card" style={{ padding: '20px 24px' }}>
          <div style={{ fontFamily: 'var(--font-display)', fontSize: '1.6rem', fontWeight: 800, marginBottom: 4 }}>{totalRegistos}</div>
          <div style={{ fontSize: '0.8rem', color: 'var(--txt-3)' }}>Total de Transaes</div>
        </div>
        <div className="card" style={{ padding: '20px 24px' }}>
      ...
      {carregando ? <PageLoader /> : (
        <div className="table-container">
          <table>
            <thead>
              <tr><th>Referência</th><th>Utilizador</th><th>Curso</th><th>Método</th><th>Valor</th><th>Estado</th><th>Data</th><th>Ações</th></tr>
            </thead>
            <tbody>
              {pagamentos.length === 0 ? (
                  ...
                  <td>
                    <div style={{ display:'flex', gap:4 }}>
                      ...
                      {['pendente','aguardando_validacao'].includes(p.status) && (
                        <button type="button"  className="btn btn--primary btn--sm" onClick={() => { setModalPag(p); setMotivo(''); }}>
                          Validar
                        </button>
                      )}
                    </div>
      ...
      {meta.totalPaginas > 1 && (
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, marginTop: 16, flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <button type="button"  className="btn btn--secondary btn--sm" disabled={meta.pagina <= 1} onClick={() => setPagina((anterior) => Math.max(1, anterior - 1))}>
              Anterior
            </button>
            ...
            <button type="button"  className="btn btn--secondary btn--sm" disabled={meta.pagina >= meta.totalPaginas} onClick={() => setPagina((anterior) => Math.min(meta.totalPaginas, anterior + 1))}>
              Seguinte
            </button>
          </div>
        </div>
      ...
      <Modal aberto={!!modalPag} onFechar={() => setModalPag(null)} titulo={`Validar Pagamento`}>
        ...
        <button type="button"  className="btn btn--primary btn--full" style={{ marginBottom:8 }} onClick={() => validar(modalPag.id, true)}>
          <Check size={15}/> Confirmar pagamento
        </button>
        <div className="form-group" style={{ marginBottom:8 }}>
          <label className="form-label">Motivo de rejeio (se aplicvel)</label>
          <textarea className="form-textarea" rows={2} value={motivo} onChange={e => setMotivo(e.target.value)} placeholder="Comprovativo ilegvel, valor incorreto..." />
        </div>
        <button type="button"  className="btn btn--danger btn--full" onClick={() => validar(modalPag.id, false)}>
    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
        {['pendente','aprovada','rejeitada','encerrada'].map(s => (
          <button type="button"  key={s} onClick={() => mudarFiltro(s)}
            className={`btn btn--sm ${filtro === s ? 'btn--primary' : 'btn--secondary'}`}
            style={{ textTransform: 'capitalize' }}>
            {s}
          </button>
        ))}
        <button type="button"  onClick={() => mudarFiltro('')} className={`btn btn--sm ${filtro === '' ? 'btn--primary' : 'btn--secondary'}`}>
          Todas
        </button>
      </div>
                ...
                <div style={{ display: 'flex', gap: 16, alignItems: 'flex-start' }}>
                  ...
                  {v.status === 'pendente' && (
                    <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
                      <button type="button"  className="btn btn--sm" style={{ background: 'var(--verde)', color: 'white' }}
                        onClick={() => aprovar(v.id, v.titulo)}>
                        <CheckCircle size={14}/> Aprovar
                      </button>
                      <button type="button"  className="btn btn--sm btn--danger" onClick={() => abrirRejeicao(v)}>
                        <X size={14}/> Rejeitar
                      </button>
                    </div>
                  )}
      ...
      <Modal aberto={!!modalRej} onFechar={() => setModalRej(null)} titulo="Rejeitar Vaga"
        acoes={<>
          <button type="button"  className="btn btn--secondary" onClick={() => setModalRej(null)}>Cancelar</button>
          <button type="button"  className={`btn btn--danger${enviando?' btn--loading':''}`} onClick={confirmarRejeicao} disabled={enviando}>
...
export function Ficheiros() {
  return <SecaoSimples icone={<Folder/>} titulo="Ficheiros" desc="Gesto de ficheiros e anexos do sistema." />;
}
  ...
  return (
    <div>
      <div className="admin-toolbar admin-toolbar--end" style={{ marginBottom: 16 }}>
        ...
        <button type="button" className="btn btn--primary btn--sm" onClick={marcarTodas} disabled={notificacoes.length === 0 || notificacoes.every((item) => item.lida)}>
          ...
          {notificacoes.map((item) => (
            <div key={item.id} className="card" style={{ padding: 18, borderLeft: item.lida ? '4px solid var(--border)' : '4px solid var(--ciano)' }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16 }}>
                ...
                {!item.lida && (
                  <button type="button"  className="btn btn--secondary btn--sm" onClick={() => marcarLida(item.id)}>
...
export function Seguranca() {
  ...
  return (
    <div>
      <p className="admin-toolbar__hint" style={{ marginBottom: 16 }}>ltimos eventos registados na plataforma (leitura).</p>
      {carregando ? <PageLoader /> : (
        <div className="table-container">
          <table>
            <thead><tr><th>Aco</th><th>Utilizador</th><th>IP</th><th>Data</th></tr></thead>
            <tbody>
              {logs.length === 0 ? (
  ...
  return (
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, alignItems: 'start' }}>
        ...
        <button type="button"  className={`btn btn--primary btn--sm${guardando?' btn--loading':''}`} style={{ marginTop:14 }} onClick={guardar} disabled={guardando}>
      ...
      <div className="card" style={{ padding:24 }}>
        ...
        <div style={{ marginTop:20, paddingTop:20, borderTop:'1px solid var(--border)' }}>
          ...
          <button type="button"  className={`btn btn--primary btn--sm${mudandoSenha?' btn--loading':''}`} onClick={alterarSenha} disabled={mudandoSenha}>
...
function ToggleRow({ label, desc, activo, onChange }) {
  return (
    <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'12px 0', borderBottom:'1px solid var(--border)' }}>
      ...
      <button type="button" role="switch" aria-checked={activo} onClick={onChange}
        style={{
